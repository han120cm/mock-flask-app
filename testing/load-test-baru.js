...
export const options = {
  ...
  //     // This is typically the best choice for global reach
  ...
  scenarios: {
    ...
    browse_main_and_images: {
      ...
      stages: [
        ...
        { duration: '1m', target: 5 }, // Stay at 50 VUs for 5 minutes
        { duration: '1m', target: 0 },  // Ramp down to 0 VUs over 2 minutes
      ],
    ...
    browse_videos: {
      ...
      startVUs: 0,
      stages: [
        { duration: '1m', target: 1 }, // Ramp up to 20 VUs over 2 minutes
        { duration: '1m', target: 1 }, // Stay at 20 VUs for 5 minutes
        { duration: '1m', target: 0 },  // Ramp down to 0 VUs over 2 minutes
      ],
      exec: 'browseVideos',
    ...
    simulate_uploads: {
      ...
      startVUs: 0,
      stages: [
        { duration: '1m', target: 1 },  // Ramp up to 5 VUs over 1 minute
        { duration: '1m', target: 1 },  // Stay at 5 VUs for 7 minutes
...
export function browseMainAndImages() {
  ...
  // 3. Fetch a specific static image from CDN - FIXED: Always make this request
...
export function browseVideos() {
  // Browse a random video category (served by App Server, assets from CDN)
  ...
  let res = http.get(`${APP_BASE_URL}/videos/${randomVideoGroup}`, {
    ...
    tags: { 
      ...
      source: 'app_server' // Page is from the app server
    },
  });
  ...
  // 2. Fetch a specific static video from CDN
  const randomVideoIndex = Math.floor(Math.random() * 7) + 33; // Corresponds to 'tutorials' group (30-59)
  res = http.get(`${CDN_BASE_URL}/static/videos/video_${randomVideoIndex}.mp4`, {
  ...
  // Even if request fails (404), we still want to measure response time
  check(res, { 
    'static video request completed': (r) => r.status === 200 || r.status === 404,
